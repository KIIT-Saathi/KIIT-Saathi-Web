...
export default function CampusMapView() {
  ...
  useEffect(() => {
    ...
    if (campusId) {
      const found = campusLocations.find(
        (c) => String(c.id) === String(campusId)
      );
      setCampusData(found || null);
    }
  }, [campusId]);
  ...
  const mapUrl = `https://www.openstreetmap.org/export/embed.html?bbox=${
    campusData.coordinates.lng - 0.015
  },${campusData.coordinates.lat - 0.015},${
    campusData.coordinates.lng + 0.015
  },${
    campusData.coordinates.lat + 0.015
  }&layer=mapnik&marker=${campusData.coordinates.lat},${campusData.coordinates.lng}`;

  const directionsUrl =
    campusData.mapsUrl ||
    `https://www.google.com/maps?q=${campusData.coordinates.lat},${campusData.coordinates.lng}`;
        ...
        <div className="lg:col-span-2">
          ...
          <div className="mt-6 p-6 bg-white/10 backdrop-blur-md text-white rounded-xl border border-white/20">
            ...
            <p className="text-sm text-white/70">
              Coordinates: {campusData.coordinates.lat.toFixed(6)},{" "}
              {campusData.coordinates.lng.toFixed(6)}
            </p>
          </div>
        </div>
