...
export default function LostAndFound() {
        ...
        for (const item of items) {
          try {
            ...
            const res = await fetch(
              `/api/lostfound/has-paid-lost-found-contact?user_id=${user.id}&item_id=${item.id}`,
              { headers }
            );
  ...
  // Payment temporarily disabled: show contact details directly
  const handleContactClick = async (item: LostFoundItem) => {
    ...
    // Prevent users from viewing their own uploaded items (optional, keep logic)
    if (user.email === item.contact_email) {
      toast({
        title: "Cannot unlock own item",
        description:
          "You cannot view contact details of an item you posted yourself.",
        variant: "destructive",
      });
      return;
    }

    // Directly mark as 'paid' so contact details are shown
    ...
    // --- Payment logic below is commented out for now ---
