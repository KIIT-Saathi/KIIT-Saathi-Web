...
const ApplicationPaymentComponent: React.FC<ApplicationPaymentComponentProps> = ({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderRes = await fetch(`/api/lostfound/create-application-unlock-order`, {
        method: 'POST',
        headers: { 
      ...
      const options = {
        ...
        handler: async (response: any) => {
          ...
          try {
            const verifyRes = await fetch(`/api/lostfound/verify-application-unlock-payment`, {
              method: 'POST',
              headers: { 
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${accessToken}`
  ...
  const paymentDialog = (
    ...
    >
      ...
      >
        <Card 
          className="w-full max-w-2xl mx-auto rounded-2xl shadow-2xl border border-border/30 overflow-hidden bg-gradient-to-br from-white to-blue-50 dark:from-gray-900 dark:to-blue-950 relative z-[10000]"
          onClick={(e) => {
            e.stopPropagation();
          }}
        >
