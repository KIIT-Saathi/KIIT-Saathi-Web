...
import { FacultyMember } from '@/app/course-structure/data/facultyData';
...
const FacultyCard = ({ faculty, isExpanded, onToggle }: FacultyCardProps) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (newUrl) {
        ...
        const img = new Image();
        img.onload = () => {
          // add cache-busting query so browser fetches latest
    ...
    <Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg border border-gray-200 bg-white dark:from-gray-900 dark:to-gray-800 rounded-xl">
      <CardContent className="p-6">
        ...
        <div className="flex justify-center mb-3">
          <Badge className="bg-gradient-to-r from-[#006400] to-[#228B22] text-white px-3 py-1 text-xs font-medium shadow-sm">
        ...
        <h3 className="text-lg font-semibold text-center mb-4 text-gray-900 dark:text-white line-clamp-2 min-h-[3.5rem] flex items-center justify-center px-2">
          {faculty.name}
          {faculty.school === 'School of Management' && faculty.designation && faculty.designation !== 'Adding Soon..'
            ? ` (${faculty.designation})`
            : ''}
        </h3>
        ...
        {isExpanded && (showEmail || showPhone || showLinkedIn) && (
          <div className="mt-5 pt-4 border-t border-gray-200 dark:border-gray-700 animate-fade-in">
            <div className="flex items-start justify-between gap-3">
              ...
              <Button size="sm" variant="ghost" onClick={hideAll} className="text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                <X className="w-4 h-4" />
              </Button>
            </div>
          </div>
