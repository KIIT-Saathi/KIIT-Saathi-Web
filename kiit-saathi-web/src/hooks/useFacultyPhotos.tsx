...
const FALLBACK_IMAGE_URL =
  'https://storage.googleapis.com/kiitsaathi.appspot.com/faculty-photos/fallback.jpg';
const LOAD_TIMEOUT = 40000; // 40 seconds timeout

/**
 * NOTE: Set NEXT_PUBLIC_HOSTED_URL in your .env.local
 * Example:
 * NEXT_PUBLIC_HOSTED_URL=https://your-api-host.com
 */
...
export function useFacultyPhotos() {
  ...
  // Next.js public env var pattern
  ...
  const preloadImage = useCallback(
    (url: string, retryCount = 0): Promise<string> => {
  ...
  const getPhotoUrl = useCallback(
    async (facultyId: string): Promise<string> => {
      if (!facultyId) {
      ...
      try {
        ...
        const response = await fetch(
          `/api/faculty/photo-url?facultyId=${encodeURIComponent(
            facultyId
          )}&t=${timestamp}`,
          {
            headers: {
            ...
            cache: 'no-store',
            // client-side fetch; credentials handled if necessary
          }
        );
        ...
        // Safe URL parsing
        let cleanUrl = String(data.photoUrl);
        try {
          const parsed = new URL(cleanUrl);
          cleanUrl = parsed.toString();
        } catch {
          // If URL constructor fails, keep the original string (may still work)
        }
  ...
  const uploadPhoto = async (facultyId: string, file: File): Promise<string | null> => {
    if (!isAdmin) {
    ...
    try {
      ...
      // Clear the cache for this faculty's photo (best-effort)
      // We don't know the exact old cached key, but try naive deletion of known forms
      const timestampless = result.photoUrl.split('?')[0];
      imageCache.forEach((value, key) => {
        if (key.includes(timestampless) || key === result.photoUrl) {
          imageCache.delete(key);
        }
      });

      // Preload the new image (cache-busted)
